...
GraphQL은 백엔드로 들어온 요청들에 대해 잘 정제된 인사이트를 제공합니다. 각 클라이언트가 어떤 정보가 필요한지 지정해서 요청하기 때문에 사용하고 있는 데이터가 어떻게 쓰이는지 깁게 이해할 수 있습니다. 예를들어 이는 전혀 쓰이지 않은 특정 필드를 버릴 수도 있고 개선할 수도 있습니다.
...
### 1.3. 핵심 개념(Core concepts)
### 1.4. Architecture
### 1.5. Clients
### 1.6. 기타등등

### 1.7. graphql-node Tutorial
...
1. `typeDef` constant는 우리의 GraphQL schema를 정의한다. 여기에서는 `Query`타입을 써서 `info`라는 하나의 필드를 갖게 선언했다. 이 필드는 `String!`이라는 타입을 갖는데, exclamation mark(느낌표)는 해당 필드가 `null`이 되면 안된다는 것을 의미한다.

2. `resolvers` 객체는 GraphQL schema의 실제적 선언이다. 그리고 이 구조는 `typeDefs`에서 선언한 것고 구조가 같다는 것을 유념하자.
...
```
실제로 GraphQL 쿼리라는 최상단 키인 query에 담긴 string형태로 전달된다. 즉
